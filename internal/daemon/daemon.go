...
func (d *Daemon) hasPendingEvents(channel, rigName string) bool {
	eventDir, err := channelevents.Directory(d.config.TownRoot, channel, rigName)
	if err != nil {
		return false
	}
	if err := channelevents.MigrateLegacyToRig(d.config.TownRoot, channel, rigName); err != nil {
		if d.logger != nil {
			d.logger.Printf("Legacy %s event migration for %s: %v", channel, rigName, err)
		}
	}
